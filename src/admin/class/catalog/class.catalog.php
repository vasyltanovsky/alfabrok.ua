<?php
/*
 * Класс обработчик каталога сайта
 *  
 *
 * @version class.catalog.php,v 1.0 2010/05/28
 * @author <AlexTsurkin/>
 * @license GNU GPLv3
 */

class CatalogHadler {
	public $Arr; #не упорядоченный массив элеменов
	public $ArrBuild; #массив данный Arr
	public $NWords; #имя текста эл.
	public $NId; #имя Id эл.
	public $NParent; #имя родителя эл.
	public $Page; #страница
	public $ArrFormation; #отворатированный древовидный массив 
	...
	/* Функция: функция формирует массив с массива, в вид радителей и детей
     *
     * @param  $this->Arr - массив всех элементов справочника
     * @param  $this->NId  
     * @param  $this->NParent 
     * @return $return - сформированный древовыдный массив
     */
	public function get_arr_formation() {
		$this->ArrFormation = array ();
		for($i = 0; $i < count ( $this->Arr ); $i ++) {
			if (empty ( $this->Arr [$i] [$this->NParent] )) {
				$this->ArrFormation [count ( $this->ArrFormation )] = array ($this->Arr [$i] [$this->NId], 'NULL', 0 );
	...
	/* Функция: вспомогательная функция для построения масива родитель и детей
     * 
     * @param  $searchID - искомый айди 
     * @param  $in - уровень вхождения
     * 
     * @param  $this->Arr - массив всех элементов справочника
     * @param  $this->NId  
     * @param  $this->NParent 
     * @return $this->ArrFormation - сформированный древовыдный массив
     */
	private function get_arr_formation_private($searchID, $in) {
		$in ++;
		for($i = 0; $i < count ( $this->Arr ); $i ++) {
			if ($this->Arr [$i] [$this->NParent] == $searchID) {
				$this->ArrFormation [count ( $this->ArrFormation )] = array ($this->Arr [$i] [$this->NId], $searchID, $in );
	...
	/* Функция: формирует развернутое древовидное меню
    * 
    * @param $this->ArrFormation - сформировованный древовидный массив 
    * @param $this->ArrBuild - массив элементов стравичников
    * @param $this->Page - 
    * @param $this->NWords -  
    * @return $return - сформированный древовыдный массив
    */
	...
	/* Функция: формирует развернутое древовидное меню JQUERY
    * 
    * @param $this->ArrFormation - сформировованный древовидный массив 
    * @param $this->ArrBuild - массив элементов стравичников
    * @param $SearchId - искомый эл. 
    * @return $return - сформированный древовыдный массив
    */
	...
	/* Функция: строка навигации
    * 
    * @param $NGet - имя массива $_GET для ориентирования
    * @param $this->NWords
    * @param $this->Page
    * @param $this->NId
    * @param $PosItem если уже выбран (позиция) элемент с каталога
    * @return $ret - строка
    */
	...
	/* Функция: строка заголовкой, ключевых слов, и т.д.
    * 
    * @param $NGet - имя массива $_GET для ориентирования
    * @param $NMeta - имя поля с массива єлементов
    * @return $ret - строка
    */
    ...
    */
	...
	/* Функция: обрабатывает от активного пункта до первого родителя
	    * 
	    * @param $NGet - имя массива $_GET для ориентирования
	    * @param $this->ArrBuild
	    * @return $ret_arr - полученный массив
	    */
	private function get_child_parent($NGet) {
		if (empty ( $_GET [$NGet] ))
			return;
		$active_id = $_GET [$NGet];
		$ret_arr = array ();
	...
	/* Функция: формирует строку для запроса с БД
    * 
    * @param $NGet - имя поля $_GET
    * @return строка
    */
	...
	/* Функция: поиска элементов которые входят 
	    * 
	    * @param $SearchID - искомый эл.
	    * @return $ret - строка
	    */
