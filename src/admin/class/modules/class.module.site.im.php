...
class ModuleSiteIm {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetDictValue($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetPropValue($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetPropListValue($data, $key) {
		//echo "<pre>";
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetPropListValueText($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetPropListWord($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetPropListImgText($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetDateValue($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetIndexImage($data, $key) {
		return $data [$key];
	}
	/* 	Функция: возвращает значение справочника по айди 
		    * 	@param $data	-	
		    * 	@param $key		-	
		    */
	private function GetCurrentPrice($data, $key) {
		return Controller::Template("application/template/immovables/price.current.php", array("data" => $data, "key" => $key));
	}
	private function GetSimplePrice($data, $key) {
		if (empty ( $data [$key] ))
			return;
		else
			return Controller::Template("application/template/immovables/price.simple.php", array("data" => $data, "key" => $key));
	}
/* 	Функция: возвращает значение справочника по айди 
		    * 	@param $data	-	
		    * 	@param $key		-	
		    */
	private function GetDataValuePrice($data, $key) {
		if (empty ( $data [$key] ))
			return;
		else
			return Discharge::GetDisValue ( $data [$key]*$_COOKIE['exchange_USD'], 4 ) . "";
	}
	
	private function GetDataValuePriceGetPriceExchangeRate ($data, $key) {
	
	}
	
	private function GetPriceExchangeRate($data, $key) {
	...
	private function GetDataValuePriceBlock($data, $key) {
	...
	/* 	Функция: возвращает другую стоимость обьекта, если аренда то продажу, и наоборот 
		    ...
		    * 	@param $key		-	
		    */
	private function GetOtherPrice($data, $key) {
		if(isset($_GET['type_cat'])) {
			$ClDisImPrice = new Discharge ();
			if(($_GET['type_cat'] == "sale") && $data['im_is_rent']) 
				return sprintf('<h4 class="im-other-price"> %s %s<span>(%s)</span></h4>', $ClDisImPrice -> GetDisValue($data["im_prace_manth"]*$_COOKIE['exchange_USD'], 4), $this->getExchangeRateBlock($data["im_prace_manth"]), $this->lang_arr['ImRent']);
			
			if(($_GET['type_cat'] == "rent") && $data['im_is_sale']) 
				return sprintf('<h4 class="im-other-price"> %s %s<span>(%s)</span></h4>', $ClDisImPrice -> GetDisValue($data["im_prace"]*$_COOKIE['exchange_USD'], 4), $this->getExchangeRateBlock($data["im_prace"]),  $this->lang_arr['ImSale']);
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetCurrentPriceNoImage($data, $key) {
		return Controller::Template("application/template/immovables/price.current.noimage.php", array("data" => $data, "key" => $key));
		/*$ClDisImPrice = new Discharge ();
		if ($data ['im_prace_old'] < $data ['im_prace']) {
			return "<span class=\"ImSpanPrice\">{$ClDisImPrice -> GetDisValue($data['im_prace'], 4)}</span><span class=\"ImSpanPriceUa\" title=\"{$this->lang_arr['exchange_by_ndu']}\">(". Discharge::GetDisValue($data['im_prace']*$_COOKIE['exchange_USD'], 4, $this->lang_arr['exchange_ua']) .")</span><span class=\"ImSpanPriceOld\">{$ClDisImPrice -> GetDisValue($data['im_prace_old'], 4)}</span>";
		}
		if ($data ['im_prace_old'] > $data ['im_prace']) {
			return "<span class=\"ImSpanPrice\">{$ClDisImPrice -> GetDisValue($data['im_prace'], 4)}</span><span class=\"ImSpanPriceUa\" title=\"{$this->lang_arr['exchange_by_ndu']}\">(". Discharge::GetDisValue($data['im_prace']*$_COOKIE['exchange_USD'], 4, $this->lang_arr['exchange_ua']) .")</span><span class=\"ImSpanPriceOld\">{$ClDisImPrice -> GetDisValue($data['im_prace_old'], 4)}</span>";
		} else {
			return "<span class=\"ImSpanPrice\">{$ClDisImPrice -> GetDisValue($data['im_prace'], 4)}</span><span class=\"ImSpanPriceUa\" title=\"{$this->lang_arr['exchange_by_ndu']}\">(". Discharge::GetDisValue($data['im_prace']*$_COOKIE['exchange_USD'], 4, $this->lang_arr['exchange_ua']) .")</span>";
		}*/
	}
	/* 	Функция: возвращает значение справочника по айди 
		    * 	@param $data	-	
		    * 	@param $key		-	
		    */
	private function GetDataValuePriceSpan($data, $key) {
		if (empty ( $data [$key] ))
			return;
	...
	private function GetPriceExchangeRateNoUsa($data, $key) {
		return "<span>" . Discharge::GetDisValue($data [$key] * $_COOKIE['exchange_USD'], 4) . "</span>";
	}
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetMap($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
		    ...
		    * 	@param $key		-	
		    */
	private function GetAdressTable($data, $key) {
	...
	private function GetStandartTable($data, $key) {
	...
	private function ArrGetSomeDictTr($data, $ArrTr) {
	...
	private function GetFullAdress($data, $key) {
	...
	private function ArrGetSomeDataTr($data, $ArrTr) {
		$ClDisImPrice = new Discharge ();
		foreach ( $ArrTr as $key => $value ) {
			if (! empty ( $data [$key] )) {
				switch ($key) {
										case 'im_prace_day' :
						{
							...
							<td class=\"some-width\">" . Discharge::GetDisValue($this->GetDataValue($data, $key)*$_COOKIE['exchange_USD'], 4) ."</td></tr>";
							break;
						}
					case 'im_prace_manth' :
						{
	...
	/* 	Функция: возвращает значение справочника по айди 
			...
			* 	@param $key		-	
			 */
	private function GetCatIm($data, $key) {
	...
	/* 	Функция: возвращает значение справочника по айди 
			...
			* 	@param $key		-	
			 */
	private function GetLinkIm($data, $key) {
		if ($data ['im_is_sale'])
	...
	/* 	Функция: возвращает значение справочника по айди 
			...
			* 	@param $key		-	
			 */
	private function GetRentIm($data, $key) {
		if ($data ['im_is_rent'])
	...
	/* 	Функция: возвращает значение справочника по айди 
			...
			* 	@param $key		-	
			 */
	private function GetSaleIm($data, $key) {
		if ($data ['im_is_sale'])
