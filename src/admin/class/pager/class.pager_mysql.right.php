...
class pager_mysql_right extends pager_right {
	...
	// Конструктор
	public function __construct($tablename, $where = "", $order = "", $pnumber = 10, $page_link = 3, $parameters = "", $parameters_after = "") {
...
abstract class pager_right {
	...
	public function __toString() {
		if ($_SERVER [PHP_SELF] != "/immovables")
		...
		if ($page - $this->get_page_link () > 1) {
			$return_page .= "<a href=\"$_SERVER[PHP_SELF]" . "{$this->get_parameters()}/1.html{$this->get_parameters_after()}\" class=\"main_txt_lnk\">1-{$this->get_pnumber()}</a>&nbsp;&nbsp;...&nbsp;&nbsp;";
			// Есть
			for($i = $page - $this->get_page_link (); $i < $page; $i ++) {
				$return_page .= "&nbsp;<a href=\"$_SERVER[PHP_SELF]" . "{$this->get_parameters()}/$i.html{$this->get_parameters_after()}\" class=\"main_txt_lnk\">" . (($i - 1) * $this->get_pnumber () + 1) . "-" . $i * $this->get_pnumber () . "</a>&nbsp;";
			}
		} else {
			// Нет
			for($i = 1; $i < $page; $i ++) {
		...
		if ($page + $this->get_page_link () < $number) {
			...
			$return_page .= "&nbsp;...&nbsp;&nbsp;" . "<a href=\"$_SERVER[PHP_SELF]" . "{$this->get_parameters()}/$i.html{$this->get_parameters_after()}\" class=\"main_txt_lnk\">" . (($number - 1) * $this->get_pnumber () + 1) . "-{$this->get_total()}</a>&nbsp;";
		} else {
			// Нет
			for($i = $page; $i <= $number; $i ++) {
				if ($number == $i) {
					if ($page == $i)
						...
						$return_page .= "&nbsp;<a href=\"$_SERVER[PHP_SELF]" . "{$this->get_parameters()}/$i.html{$this->get_parameters_after()}\" class=\"main_txt_lnk\">" . (($i - 1) * $this->get_pnumber () + 1) . "-{$this->get_total()}</a>&nbsp;";
				} else {
					if ($page == $i)
						$return_page .= "&nbsp;" . (($i - 1) * $this->get_pnumber () + 1) . "-" . $i * $this->get_pnumber () . "&nbsp;";
					else
