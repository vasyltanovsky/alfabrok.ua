...
require_once DOC_ROOT . '/config/class.config.php';
	...
	if($_GET['news_id'])
	{
		# 	Получаем содержимое текущей страницы
			...
			$active_id = $cl_sel_pages -> select_table_id("WHERE news_id = '$_GET[news_id]'");
	}
...
?>
   ...
   <script>
	...
	function mycallbackform(v,m,f){
		if(v == 'dell')
		{
			//$("#loading").ajaxStart(function(){
  			//$(this).show();});
			$('#DivRequestImg').load('template.dell.position.item.img.php?news_id=<?php echo $active_id['news_id'];?>&pp_img=<?php echo $active_id['news_image'];?>');
    ...
    <div id="d-filter">
	<?php
			 echo "<img src=\"../../files/news/{$active_id[news_image]}\">";
	?>
    </div>
