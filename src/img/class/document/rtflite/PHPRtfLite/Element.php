...
class PHPRtfLite_Element {
	...
	public static function convertTagsToRtf($text, $charset) {
		$search = array (// bold
		'/<STRONG[ ]*>(.*?)<\/STRONG[ ]*>/smi', '/<B[ ]*>(.*?)<\/B[ ]*>/smi', // italic
		'/<EM[ ]*>(.*?)<\/EM[ ]*>/smi', '/<I[ ]*>(.*?)<\/I[ ]*>/smi', // underline
		'/<U[ ]*>(.*?)<\/U[ ]*>/smi', // break
		'/<BR[ ]*(\/)?[ ]*>/smi', '/<LINE[ ]*(\/)?[ ]*>/smi', // horizontal rule
		'/<HR[ ]*(\/)?[ ]*>/smi', '/<CHDATE[ ]*(\/)?[ ]*>/smi', '/<CHDPL[ ]*(\/)?[ ]*>/smi', '/<CHDPA[ ]*(\/)?[ ]*>/smi', '/<CHTIME[ ]*(\/)?[ ]*>/smi', '/<CHPGN[ ]*(\/)?[ ]*>/smi', // tab
		'/<TAB[ ]*(\/)?[ ]*>/smi', // bullet
		'/<BULLET[ ]*(\/)?[ ]*>/smi', '/<PAGENUM[ ]*(\/)?[ ]*>/smi', '/<SECTNUM[ ]*(\/)?[ ]*>/smi' )//            '/<PAGE[ ]*(\/)?[ ]*>/smi',
		//            '/<SECT[ ]*(\/)?[ ]*>/smi'
		;
		
		$replace = array (// bold
		'\b \1\b0 ', '\b \1\b0 ', // italic
		'\i \1\i0 ', '\i \1\i0 ', // underline
		'\ul \1\ul0 ', // break
		'\line ', '\line ', // horizontal rule
		'{\pard \brdrb \brdrs \brdrw10 \brsp20 \par}', '\chdate ', '\chdpl ', '\chdpa ', '\chtime ', '\chpgn ', // tab
		'\tab ', // bullet
		'\bullet ', '\chpgn ', '\sectnum ' )//            '\page ',
		//            '\sect '
		;
