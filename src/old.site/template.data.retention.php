...
if (isset ( $_GET ['action'] )) {
	...
	if ($_GET ['action'] == 'getArchive') {
		...
		{
			$old = $path.'st_'.$foto['im_photo_id'].'.'.$foto['im_file_type'];
			$new = 'temp/'.$t.'/'.$i.'.jpg';
			$i += 1;
			@copy($old, $new);				
		}
		...
		$archive->add('temp/'.$t,PCLZIP_OPT_REMOVE_PATH, 'temp');
...
if ($_POST ['retention'] == 'edit_page') {
	...
	$arr_update = array ("susr_id" => $_POST[susr_id].",", "im_catalog_id" => "'{$_POST[im_catalog_id]}',", "im_type_id" => "'{$_POST[im_type_id]}',", "im_array_id" => "'{$_POST[im_array_id]}',", "im_region_id" => "'{$_POST[im_region_id]}',", "im_a_region_id" => "'{$_POST[im_a_region_id]}',", "im_city_id" => "'{$_POST[im_city_id]}',", "im_area_id" => "'{$_POST[im_area_id]}',", "im_adress_id" => "'{$_POST[im_adress_id]}',", "im_adress_house" => "'{$_POST[im_adress_house]}',", "im_prace" => "{$_POST[im_prace]},", "im_prace_old" => "{$_POST[im_prace_old]},", "im_prace_sq" => "{$_POST[im_prace_sq]},", "im_prace_day" => "{$_POST[im_prace_day]},", "im_prace_manth" => "{$_POST[im_prace_manth]},", "im_space" => "'{$_POST[im_space]}',", "im_space_value_id" => "'{$_POST[im_space_value_id]}',", "im_sale_id" => "'{$_POST[im_sale_id]}',", "im_provider" => "'{$_POST[im_provider]}',", "pos" => "{$_POST[pos]},", "im_is_hot" => "{$im_is_hot},", "im_is_sale" => "{$im_is_sale},", "im_is_rent" => "{$im_is_rent},", "im_is_special" => "{$im_is_special},", "hide" => "'{$hide}',", "im_adress_flat" => "'{$_POST[im_adress_flat]}',", "im_title" => "'{$_POST[im_title]}',", "im_geopos" => "'{$_POST[im_geopos]}',", "im_code" => "'{$_POST[im_code]}',", "im_date_add" => "'{$_POST[im_date_add]}'" );
...
if ($_POST ['retention'] == 'add_img') {
	...
	if ($_FILES ['Filedata'])
		if (copy ( $_FILES ['Filedata'] ['tmp_name'], $fileDir . '' . $fileName )) {
			...
			$active_id = $ClUserIm->select_table_id ( "WHERE im_id = {$_POST[im_id]} ORDER BY im_id DESC LIMIT 1" );
			#
			$query = "INSERT INTO {$tbl_im_ph} (`im_photo_id`, `im_photo_type`, `im_id`, `im_photo_order`, `im_file_type`) VALUES
													 	('{$photoId}', '{$_POST[im_photo_type]}', {$_POST[im_id]}, NULL, '{$extension}');";
