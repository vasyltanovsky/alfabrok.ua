<div class="rlist rListItem-<?php echo $margin; ?> parent-element-<?php echo $parent_id;?>" style="margin:0 0 0 <?php echo ($margin ? $margin : 0)*10; ?>px; <?php echo (($parent_id == 0 ? "false" : $is_checked) ? "" : "display:none;"); ?>">
	<?php foreach ($list as $key => $value) :?>
		<div class="item">
			...
			<?php 
				if($value["child"]) {
					echo Controller::Template("/application/includes/search/regional.search.block.item.php", array("list"=> $value["child"], "Data" => $Data, "margin" => $value[2] + 1, "parent_id" => $value["0"], "is_checked" => $value["checked"], "isKiev" => $isKiev));
				}
			?>
		</div>
	<?php endforeach;?>
