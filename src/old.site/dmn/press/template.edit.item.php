...
<?php
	...
	#объявляем класс словаря
		...
		$dct		=	 $dictionaries->buid_dictionaries($tbl_dictionaries,
									 	 				  "WHERE lang_id = {$_COOKIE['lang_id']} AND parent_id !=  ''");
	...
	#	функция формирует списов возможный родителей, справочник меню
		function sel_parent_id($arr, $sel = 'NULL', $name_id = 'news_id', $echo_id = 'menu_words')
		{
			...
			return $str;
                ...
                #	преобразуем флажки
								$active_id = $cl_sel_pages -> buld_table[$_POST['news_id']];
								$hide = "checked=\"checked\"";
								if($active_id['hide'] == 'hide') $hide = '';
								$IsShowIndex = "checked=\"checked\"";
								if(empty($active_id['is_show_index'])) $IsShowIndex = '';
								
			#	img
			if(empty($active_id['news_image']))
			{
