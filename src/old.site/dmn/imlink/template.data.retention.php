  ...
  $response['fieldErrors'] = array();
 	...
 	if($_POST['retention'] == 'edit_page')
	{
			$arr_update 		 = array("type_rs"	 		=> "'{$_POST['type_rs']}',",
										 ...
										 "il_title" 		=> "'{$_POST['il_title']}',",
										 "il_text" 			=> "'{$_POST['il_text']}',",
										 "dict_id" 			=> "'{$_POST['dict_id']}'");
	
			$cl_page_update  = new mysql_select($tbl_im_link);
			$cl_page_update	->update_table("WHERE il_id = '{$_POST['il_id']}' AND lang_id = {$_COOKIE['lang_id']}",
											$arr_update);
