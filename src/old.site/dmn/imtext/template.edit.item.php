...
<?php 
	...
	#	функция формирует списов возможный родителей, справочник меню
		...
		# 	Получаем содержимое текущей страницы
   		$cl_sel_pages = new mysql_select($tbl_im_text);
		$active_id = $cl_sel_pages -> select_table_id("WHERE lang_id = $_COOKIE[lang_id] and it_id='{$_POST['it_id']}'");
		
		$type_rent = "";
		$type_sale = "";
		if ($active_id[$i]['type_rs'] == 'rent')
			 $type_rent = "selected=\"selected\"";
		if ($active_id[$i]['type_rs'] == 'sale')
			 $type_sale = "selected=\"selected\"";	  	
			//echo "<pre>";
			//print_r($BuildResult);
			//echo "</pre>";
