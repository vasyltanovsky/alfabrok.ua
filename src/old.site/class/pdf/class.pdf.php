...
class Cpdf {
		...
		if (! isset ( $this->fonts [$fontName] )) {
			...
			if (isset ( $this->fonts [$fontName] )) {
				...
				if (substr ( $fontName, - 4 ) == '.afm' && strlen ( $fbtype )) {
					...
					$fdopt = array ('Flags' => $flags, 'FontName' => $adobeFontName, 'StemV' => 100 );// don't know what the value for this should be!
	...
	function saveState($pageEnd = 0) {
		if ($pageEnd) {
			...
			$this->nStateStack ++;
			$this->stateStack [$this->nStateStack] = array ('col' => $this->currentColour, 'str' => $this->currentStrokeColour, 'lin' => $this->currentLineStyle );
		}
		$this->objects [$this->currentContents] ['c'] .= "\nq";
	}
	...
	function addPngFromFile($file, $x, $y, $w = 0, $h = 0) {
		...
		$this->o_image ( $this->numObj, 'new', array ('label' => $label, 'data' => $idata, 'bitsPerComponent' => $info ['bitDepth'], 'pdata' => $pdata, 'iw' => $info ['width'], 'ih' => $info ['height'], 'type' => 'png', 'color' => $color, 'ncolor' => $ncolor ) );
