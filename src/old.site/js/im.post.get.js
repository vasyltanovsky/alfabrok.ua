...
$(document).ready(function(){
		...
		//var Ajaxload = "/report_center.php?action=set_friend_im&im_id=29&name=alex&email=a.tsurkin@avtologistika.com"; 
...
function SendImToFriendIsTrue(v,m,f){
	...
	var Ajaxload = "/report_center.php?action=set_friend_im&im_id="+Fim_id+"&name="+FFriendName+"&email="+FemailFriend; 
	$('#DivRequestQuery').load(Ajaxload);
...
function setStylePnumber(Fid,FValue) {
	$('#DivRequest').load('/application/module/immovables/get.post.hadler.php?action=set_style_show&'+Fid+'='+FValue+'');
...
function setSortTable(FValue) {
	$('#DivRequest').load('/application/module/immovables/get.post.hadler.php?action=set_sort_table&sort='+FValue+'');
	//alert('asd');
	$("#DivRequest").ajaxComplete(function(){
  		//location.href = location.href.toLowerCase();
		location.href = location.href;
