...
<table cellpadding="0" cellspacing="0" border="0" class="list_Flat_table_block">
    <tr class="list_Flat_table_block_header">
        ...
        <td class="ImFListHeaderM2"><a rel="nofollow" href="javascript:setSortTable('im_prace_sq');" class="<?php echo WhatSort("im_prace_sq");?>"><?php echo getLangString('ImFListHeaderSotku');?> ($)</a></td>
    ...
    <tr>
        ...
        <td class="ListTableImIndexImg"><a href="/ru/land/sale/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>"><img src="<?php echo getLangString('imageDomain');?>/files/images/immovables/st_<?php echo $value["im_photo"];?>" alt="<?php echo $value["im_title"];?>"/></a></td>
        <td class="TdListLeftAlight"><p><?php echo $value["im_title"];?></p><p><b><?php echo $m->GetFullAdress($value, "im_full_adress")?></b></p><div class="ImPropTable"><?php echo $m->GetPropListValue($value, "im_prop_list")?></div><a class="AReadMore" href="/ru/land/sale/<?php echo $routingObj->getAction();?>/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>"><?php echo getLangString(REadMore);?></a></td>
    	...
    	<td class="TdListRightBorder"><a href="/ru/land/sale/a<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>#YMapsID"><?php echo appHtmlClass::partial("immovables/ymap/image", array("Model"=>$Model, "item" => $value, "m" => $m) );?></a></td>	
	</tr>
	<?php endforeach;?>
</table>