 ...
 * (and other search engines) indexes your page.
...
class SiteMapGenerator {
	 ...
	 */
	public function __construct($site, $navigate = true, $noFollowWords = array()) {
		$this->site = $site;
		$this->hash = array ();
		$this->noFollowWords = $noFollowWords;
		if ($navigate) {
			$this->link ( $site );
		}
	}
	...
	public function link($link) {
		...
		foreach ( $this->noFollowWords as $key => $value ) {
	...
	public function navigate() {
		$this->html = "";
		if (file_get_contents ( $this->actual ))
			;
	...
	public function generateSiteMap() {
		$xml = new SimpleXMLElement ( "<?xml-stylesheet type=\"text/xsl\" href=\"" . $this->site . "/application/module/SitemapGenerator/gsitemaps/gss.xsl\"?><urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\"></urlset>" );
		$lastmod = date ( "Y-m-d G:i" );
